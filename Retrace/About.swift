...
class About {
  ...
  private var links: NSMutableAttributedString {
    let string = NSMutableAttributedString(string: "Website│GitHub│Support",
                                           attributes: [NSAttributedString.Key.foregroundColor: NSColor.labelColor])
    string.addAttribute(.link, value: "https://example.com/Retrace", range: NSRange(location: 0, length: 7))
    string.addAttribute(.link, value: "https://example.com/Retrace/source", range: NSRange(location: 8, length: 6))
    string.addAttribute(.link, value: "mailto:support@example.com", range: NSRange(location: 15, length: 7))
    return string
  }
